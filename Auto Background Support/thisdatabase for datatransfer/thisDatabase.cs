...
namespace Accounting_Application_System
{
    public class thisDatabase
    {
        public static String driveloc = "\\\\RIGHTAPPS\\RightApps\\";//"\\\\RA-PDAC\\RightApps\\";// 
         ...
         public static String lcl_db = "inv_balai_ahsi080117";
        ...
        {
            thisDatabase db = new thisDatabase();
            DataTable dt = db.QueryOnTableWithParams("m99", "branch", "", "");
            Boolean flag = false;
            ...
            try
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if(dt.Rows[i]["branch"].ToString() == "Y")
                    {
                        flag = true;
                    }
                }
        ...
        public Boolean InsertOnTable(String table, String column, String value)
        {
            ...
            {
                String col = "tablename,col,val,cond,action,transfercode";
                String action = "insert";
                String condition = "";
                String transcode = get_pk("transfercode");
                String final_value = thisDatabase.Encrypt(value, "sblw-3hn8-sqoy19");
                String val = "'" + table + "','" + column + "','" + final_value + "','" + condition + "','" + action + "','" + transcode + "'";
                try
                {
                    thisDatabase2 db2 = new thisDatabase2();

                    if (db2.InsertOnTable("transferdata", col, val)) {
                       // get_nextincrementlimitchar(transcode, 8);
                        this.set_pkm99("transfercode", this.get_nextincrementlimitchar(transcode, 8));
        ...
        public Boolean UpdateOnTable(String table, String col_upd, String cond)
        {
            ...
            if (GlobalClass.isMainBranch == false)
            {
                ...
                String condition = thisDatabase.Encrypt(cond, "sblw-3hn8-sqoy19");
                String value = "";
                String final_value = thisDatabase.Encrypt(col_upd, "sblw-3hn8-sqoy19");
                String transcode = get_pk("transfercode");
                String val = "'" + table + "','" + condition + "','" + value + "','" + final_value + "','" + action + "','" + transcode + "'";
                try
                {
                    thisDatabase2 db2 = new thisDatabase2();
                    if(table!="m99")
        ...
        public Boolean DeleteOnTable(String table, String cond)
        {
            ...
            if (GlobalClass.isMainBranch == false)
            {
                ...
                String transcode = get_pk("transfercode");
                String val = "'" + table + "','" + column + "','" + value + "','" + condition + "','" + action + "','" + transcode + "'";
                try
                {
                    thisDatabase2 db2 = new thisDatabase2();
        ...
        {
            return this.QueryBySQLCode("SELECT p.pret_num, p.supl_code, p.supl_name, p.reference, TO_CHAR(p.trnx_date, 'MM/DD/YYYY') AS trnx_date, p.whs_code, p.jrnlz, p.cancel, p.user_id, TO_CHAR(p.t_date, 'MM/DD/YYYY') AS t_date, p.t_time FROM " + schema + ".prethdr p LEFT JOIN " + schema + ".m07 s ON s.c_code=p.supl_code LEFT JOIN " + schema + ".whouse w ON w.whs_code=p.whs_code WHERE p.t_date between " + "'" + frm.ToString("yyyy-MM-dd") + "' AND '" + to.ToString("yyyy-MM-dd") + "' ORDER BY pret_num");
        ...
        public DataTable get_stkinvlist(DateTime frm, DateTime to, String typ) //for stocks alone
        {
            ...
            return this.QueryBySQLCode("SELECT p.payment_term, p.\"locationFrom\", p.\"locationTo\", p.rec_num, p.supl_name,p.supl_code, p._reference, TO_CHAR(p.trnx_date, 'MM/DD/YYYY') AS trnx_date, p.whs_code, p.trn_type, p.jrnlz, p.cancel, p.recipient, TO_CHAR(p.t_date, 'MM/DD/YYYY') AS t_date, p.t_time, p.vat_code, w.whs_desc, p.purc_ord, p.printed FROM " + schema + ".rechdr p LEFT JOIN " + schema + ".whouse w ON w.whs_code=p.whs_code WHERE p.t_date between " + "'" + frm.ToString("yyyy-MM-dd") + "' AND '" + to.ToString("yyyy-MM-dd") + "' ORDER BY rec_num");
